<template>
  <n-card>
    ...
    <!--新建任务的modal-->
    <n-modal v-model:show="showAddModal" :show-icon="false" preset="dialog" title="新建任务">
      <template #action>
        <n-space>
...
import {getTasks, deleteTask, addTask} from "@/api/tasks/task"
...
const loadTaskData = async () => {
  try {
    ...
    if (response.status === 200) {
      ...
      taskDates.value = response.data.items
    }
  } catch (error) {
    console.log(error)
  }
...
const columns = createColumns({
  infoRow(rowData) {
    ...
    router.push({name: 'task-record', params: {token: rowData.task_token}})
  },
  deleteRow(rowData) {
    // console.log(rowData.task_token)
    n_dialog.warning({
