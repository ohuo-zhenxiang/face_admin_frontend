    ...
    <n-layout :inverted="inverted">
      ...
      <n-layout-content class="layout-content" :class="{'layout-default-background': getDarkTheme === false}">
        <div class="layout-content-main"
          ...
          <!-- TabsView 类似面包屑那种，用不用？？？-->

          <div class="main-view"
               :class="{'main-view-fix': fixedMulti, noMultiTabs: !isMultiTabs, 'mt-3': !isMultiTabs,}">
            <MainView/>
          </div>
        </div>

        <NLayoutFooter v-show="false">
          <PageFooter/>
        </NLayoutFooter>
      </n-layout-content>
...
const fixedMenu = computed(() => {
  const fixed: boolean = unref(headerSetting.value.fixed);
  return fixed ? "absolute" : "static";
});
